...
const testimonials = [
  ...
  { quote: "As a diaspora Kenyan, learning Swahili here helped me rebuild that bond with my family I had lost for years.", name: "Aisha N.", role: "Software Engineer, USA", initials: "AN", color: "bg-teal-300" },
  { quote: "My company expanded into East Africa and I needed business Swahili fast. Professional, focused, and effective.", name: "Marcus L.", role: "Business Development, Netherlands", initials: "ML", color: "bg-teal-300" },
];
...
const heroTeachers = [
  ...
  { initial: "F", name: "Fatuma Mwangi", spec: "Kids & Beginners", rating: 4.8, reviews: 201, price: "$18", gradient: "from-teal-400 to-purple-600" },
];

export default async function HomePage() {
  const [featuredTeachers, latestPosts, supabase] = await Promise.all([
    getFeaturedTeachers(),
    getBlogPosts(3),
    createClient(),
  ]);
  ...
  return (
    <PageWrapper>
      ...
      {/* ── HERO ── */}
      <section className="relative bg-gradient-to-b from-teal-50/60 to-white overflow-hidden" aria-label="Hero">
        {/* Decorative shapes */}
        <div aria-hidden="true" className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-20 -right-20 w-96 h-96 rounded-full bg-teal-100/50" />
          <div className="absolute top-20 right-48 w-64 h-64 rounded-full bg-teal-200/30" />
          <div className="absolute -bottom-24 right-24 w-80 h-80 rounded-full bg-amber-100/30" />
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div>
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black font-heading leading-[1.0] tracking-[-0.03em] text-slate-900 mb-6">
                Learn Swahili<br />
                from{" "}
                <span className="text-teal-600">Native<br />Speakers</span>
              </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-10">
                <Link href="/teachers">
                  <button className="px-8 py-3.5 bg-teal-600 hover:bg-teal-700 text-white font-bold text-base rounded-lg transition-all active:scale-95 flex items-center gap-2">
                    Find a Teacher <ArrowRight className="h-4 w-4" />
                  </button>
                </Link>
                <Link href="/become-a-teacher">
                  <button className="px-8 py-3.5 bg-white hover:bg-slate-50 text-slate-800 font-bold text-base rounded-lg border border-slate-200 transition-all">
                    Become a Teacher
                  </button>
                </Link>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2" aria-hidden="true">
                  {["#0d9488","#14b8a6","#f59e0b","#f472b6","#34d399"].map((c) => (
                    <div key={c} className="h-8 w-8 rounded-full ring-2 ring-white" style={{ background: c }} />
                  ))}
                </div>
            ...
            <div className="hidden lg:flex justify-center items-center" aria-hidden="true">
              <div className="relative w-80 h-[460px]">
                <div className="absolute inset-0 bg-teal-200/20 rounded-full blur-3xl scale-90" />
                {heroTeachers.map((t, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`h-11 w-11 rounded-lg bg-gradient-to-br ${t.gradient} flex items-center justify-center text-white font-bold shrink-0`}>{t.initial}</div>
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between">
                      <span className="text-xs bg-teal-50 text-teal-700 rounded-full px-2.5 py-1 font-medium">Native speaker</span>
      ...
      <section className="py-24 bg-slate-50" aria-labelledby="goals-heading">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              {goals.map((goal) => (
                <Link
                  key={goal}
                  href="/teachers"
                  className="px-6 py-3 bg-white border border-slate-200 rounded-lg font-semibold text-slate-800 hover:border-teal-400 hover:text-teal-700 hover:bg-teal-50 transition-all text-sm"
                >
                  {goal}
                </Link>
              ))}
              <Link
                href="/teachers"
                className="px-6 py-3 bg-slate-100 border border-slate-200 rounded-lg font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600 transition-all text-sm"
              >
                All specialisations →
              </Link>
            </div>
              ...
              <ScrollReveal key={name} delay={i * 100}>
                <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 flex flex-col">
                  ...
                  <div className={`relative h-40 ${color} flex items-end justify-center pb-0 overflow-hidden`}>
                    <div className="absolute bottom-0 translate-y-1/2 h-20 w-20 rounded-2xl bg-teal-600 flex items-center justify-center text-white font-black text-2xl shadow-lg z-10">
                      {initials}
                    </div>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-slate-50 rounded-3xl p-8 h-full">
                  ...
                  <div className="h-40 mb-8 flex items-center justify-center relative">
                    <div className={`w-28 h-28 rounded-full flex items-center justify-center ${
                      i === 0 ? "bg-amber-300" : i === 1 ? "bg-teal-300" : "bg-teal-300"
                    }`}>
          ...
          <div className="text-center mt-10">
            <Link href="/how-it-works">
              <button className="px-8 py-3 bg-white border border-slate-200 rounded-lg font-semibold text-slate-700 hover:border-teal-300 hover:text-teal-600 transition-all text-sm">
                Learn how it works →
              </button>
            </Link>
          </div>
      ...
      {/* ── FEATURED TEACHERS ── */}
      <section className={`bg-slate-50 ${featuredTeachers.length > 0 ? "py-24" : "py-16"}`} aria-labelledby="teachers-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 mb-12">
            ...
            <Link href="/teachers">
              <button className="px-6 py-3 bg-white border border-slate-200 rounded-lg font-semibold text-slate-700 hover:border-teal-300 hover:text-teal-600 transition-all text-sm shrink-0">
                Browse all teachers →
              </button>
            </Link>
          </ScrollReveal>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {featuredTeachers.map((teacher, i) => (
                <ScrollReveal key={teacher.id} delay={i * 80}>
                  <TeacherCard teacher={teacher} isAuthenticated={isAuthenticated} />
                </ScrollReveal>
              ))}
            </div>
          ) : (
            <div className="bg-white rounded-3xl p-12 text-center">
              <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-teal-50 mb-5">
              ...
              <Link href="/become-a-teacher">
                <button className="px-6 py-3 bg-teal-600 text-white font-semibold rounded-lg hover:bg-teal-700 transition-all text-sm">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <ScrollReveal className="relative">
              <div className="relative h-80 flex items-center justify-center">
                ...
                <div className="absolute w-48 h-48 rounded-full bg-teal-200/50 translate-x-16 translate-y-8" />
                <div className="relative z-10 bg-white rounded-2xl shadow-xl p-6 max-w-xs">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="h-12 w-12 rounded-lg bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-lg shrink-0">A</div>
                    <div>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <span className="text-xs bg-teal-50 text-teal-600 rounded-full px-3 py-1 font-medium">Native speaker</span>
            ...
            <ScrollReveal delay={100}>
              ...
              <Link href="/teachers">
                <button className="px-8 py-3.5 bg-teal-600 text-white font-bold rounded-lg hover:bg-teal-700 transition-all flex items-center gap-2 text-sm">
                  Find a teacher <ArrowRight className="h-4 w-4" />
                </button>
              </Link>
            </ScrollReveal>
      ...
      {/* ── FINAL CTA ── */}
      <section className="py-32 bg-slate-50 relative overflow-hidden" aria-labelledby="cta-heading">
        <div aria-hidden="true" className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -left-20 top-0 w-80 h-80 rounded-full bg-teal-300/40" />
          <div className="absolute -right-20 bottom-0 w-96 h-96 rounded-full bg-amber-300/40" />
          <div className="absolute left-1/3 top-1/2 -translate-y-1/2 w-48 h-48 rounded-full bg-teal-200/30" />
        </div>
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <ScrollReveal>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
              <Link href="/teachers">
                <button className="px-10 py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold text-base rounded-lg transition-all active:scale-95 flex items-center gap-2 justify-center">
                  Find a Teacher <ArrowRight className="h-4 w-4" />
                </button>
              </Link>
              <Link href="/contact">
                <button className="px-10 py-4 bg-white hover:bg-slate-50 text-slate-800 font-bold text-base rounded-lg border border-slate-200 transition-all">
                  Contact Us
                </button>
              </Link>
            </div>
